...
const TENCENT_FIXTURE = {
  ...
  data: {
    sh600000: {
      qfqday: [
        ['2026-07-23', '8.920', '9.050', '9.060', '8.910', '50118900.000'],
        ['2026-07-24', '9.080', '9.040', '9.120', '9.020', '50675100.000'],
        ['2026-07-27', '9.000', '9.050', '9.130', '8.960', '55723100.000'],
      ],
      // 同时提供 day（不复权），确保 raw 默认模式下解析不会抛 "无 qfqday/day 数组"
      day: [
        ['2026-07-23', '8.920', '9.050', '9.060', '8.910', '50118900.000'],
        ['2026-07-24', '9.080', '9.040', '9.120', '9.020', '50675100.000'],
        ['2026-07-27', '9.000', '9.050', '9.130', '8.960', '55723100.000'],
...
describe('URL 构造', () => {
  it('腾讯：qfq 前复权参数', () => {
    expect(buildTencentUrl('600000', 120, 'qfq')).toBe(
  ...
  it('腾讯 raw 模式：URL 末位无 qfq 参数', () => {
    ...
    // 默认 raw 模式不应有 qfq（DB 默认是不复权）
    expect(buildTencentUrl('600000', 30)).not.toContain(',qfq');
    // 显式 qfq 模式应有 qfq
    expect(buildTencentUrl('600000', 30, 'qfq')).toContain(',qfq');
  });
...
describe('parseTencent', () => {
  it('字段序是 [date,open,close,high,low,volume]（open/close 在 high/low 前）', () => {
    ...
    expect(bars[0]).toEqual({
      ...
      high: 9.06,
      low: 8.91,
      volume: 50.12, // 腾讯原生「股」÷1000000 → 万手（四舍五入2位）
      amount: 0,
    });
  });
  ...
  it('返回的 bars 按日期升序且无重复', async () => {
    const dup = {
      ...
      data: {
        sh600000: {
          qfqday: [
            ['2026-07-24', '9.080', '9.040', '9.120', '9.020', '50675100'],
            ['2026-07-23', '8.920', '9.050', '9.060', '8.910', '50118900'],
            ['2026-07-24', '9.080', '9.040', '9.120', '9.020', '50675100'], // 重复
          ],
          // 同步提供 day 字段（raw 默认模式需要）
          day: [
            ['2026-07-24', '9.080', '9.040', '9.120', '9.020', '50675100'],
            ['2026-07-23', '8.920', '9.050', '9.060', '8.910', '50118900'],
            ['2026-07-24', '9.080', '9.040', '9.120', '9.020', '50675100'],
